...
export default function FriendRequests() {
  ...
  const allRequests = useSelector(selectFriendRequests);
  const [showAllRequests, setShowAllRequests] = useState(false);

  // Show only first 3 requests initially, or all if showAllRequests is true
  const displayedRequests = showAllRequests ? allRequests : allRequests.slice(0, 3);
  ...
  return (
    <div className="rounded-lg p-4 bg-white w-full">
          ...
          displayedRequests.map((request) => (
            <div key={request.id} className="flex items-center justify-between py-3 last:border-0">
              ...
              <div className="flex items-center gap-1.5">
                <Button 
                  className="cursor-pointer bg-transparent w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center border border-[#FA8171] text-red-500 rounded-full hover:bg-red-200"
                  onClick={()=> handleDeclineRequest(request)}
                >
                  <X className="h-4 w-4 sm:h-5 sm:w-5" />
                </Button>
                <Button 
                  className="cursor-pointer w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center bg-[#4AB794] rounded-full hover:bg-green-200"
      ...
      {allRequests.length > 3 && (
        <Button 
          className="w-full mt-4 text-black py-7 cursor-pointer bg-transparent border border-[#28303F] text-[16px] font-medium rounded-lg hover:bg-gray-100"
          onClick={toggleShowAllRequests}
        >
          {showAllRequests ? 'Load Less' : 'Load More'}
        </Button>
      )}
    </div>
  );
