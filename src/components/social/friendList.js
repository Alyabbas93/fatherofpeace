...
import { nextPage, prevPage, selectCurrentPage, selectDisplayFriends, selectTotalPages } from "@/store/slices/friendsSlice";
...
const FriendList = () => {
  ...
  const friends = useSelector(selectDisplayFriends);
  const currentPage = useSelector(selectCurrentPage);
  const totalPages = useSelector(selectTotalPages);
  const allFriends = useSelector((state) => state.friends.friendsList);
          ...
          <div className="grid lg:grid-cols-2 gap-8 ">
            {friends.map((friend) => (
              <FriendCard
                ...
                avatarSrc={friend.avatarSrc || friend.avatar}
              />
            ))}
          </div>

          <div className="flex items-end lg:gap-6 justify-center p-2">
            <div className={` cursor-pointer ${currentPage === 0 ?"opacity-30" : "opacity-100"}`} 
            onClick={goToPreviousPage}>
              ...
              >
                <path
                  ...
                  fill={currentPage === 0 ? "#EAEAEA" : "#828282"}
                />
              </svg>
            </div>

            <div className={` cursor-pointer ${currentPage >= totalPages -1 ? "opacity-30": "opacity-100"}`  }
             onClick={goToNextPage}>
              ...
              >
                <path
                  ...
                  fill={currentPage >= totalPages -1 ? "#EAEAEA" : "#828282"}
                />
              </svg>
            </div>
          </div>
